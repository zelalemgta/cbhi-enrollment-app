...
const TotalContributionStats = (props) => {
    ...
    useEffect(() => {
        if (props.enrollmentPeriod)
            ipcRenderer.send(channels.REPORT_TOTAL_CONTRIBUTION_STATS, props.enrollmentPeriod);
        ipcRenderer.on(channels.REPORT_TOTAL_CONTRIBUTION_STATS, (event, result) => {
            setContributionStat(result);
        });
        return () => { ipcRenderer.removeAllListeners(channels.REPORT_TOTAL_CONTRIBUTION_STATS); }
    ...
    return (
        <TableContainer className={classes.table} component={Paper}>
            <Table aria-label="Total Statistics" size="small">
                <TableHead>
                    <TableRow>
                        <TableCell>Total Contribution Stats</TableCell>
...
export default TotalContributionStats;